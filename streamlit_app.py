    ...
    def fetch_historical_candles():
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                scalers_fitted = True
                state['df_history'] = df.reset_index(drop=True)
...
st.markdown(f"**Таймфрейм:** {TIME_WINDOW_MINUTES} мин | **Нейронов:** {NEURONS} | **Фичей:** price + volume")
...
col3.metric("Прогноз LTC", f"${state['latest_pred']:.2f}", f"{state['latest_pred'] - state['latest_price']:+.2f}")
...
if points_to_show > 0:
    ...
    if not has_signals:
        st.write("На выбранном отрезке графика нет сигналов.")
else:
